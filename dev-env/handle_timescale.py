...
class TimescaleConnector:
    ...
    def get_data_for_sample_id(self, sample_id, table_name, limit=None, start=None, end=None) -> pd.DataFrame:
        conditional = "WHERE sample_id=%(sample_id)s ORDER BY \"timestamp\" ASC"
        if start:
            conditional += " AND timestamp > %(start)s"
        if end:
            conditional += " AND timestamp< %(end)s"
        if limit:
            conditional += " LIMIT %(limit)s"
        df = pd.read_sql_query(f""" --sql
                                    SELECT {table_name}.* FROM public.{table_name} 
                                    {conditional};
                                    """,
                               self.connection, params={'sample_id': sample_id,'table_name':table_name, 'limit':limit, 'start': start, 'end': end})
        return df
